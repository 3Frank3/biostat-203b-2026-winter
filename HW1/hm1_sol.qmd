...
couple days; do not leave it to the last minute.) \## Q3. Linux Shell
Commands
...
ls -l ~/mimic/physionet.org/files/mimiciv/3.1
...
```{bash}
#| eval: false
zcat < /Users/xuzhiwei/Project/UCLA_MDSH_203B/mimic/physionet.org/files/mimiciv/3.1/hosp/admissions.csv.gz | head -10
#zless /Users/xuzhiwei/Project/UCLA_MDSH_203B/mimic/physionet.org/files/mimiciv/3.1/hosp/admissions.csv.gz
#zmore /Users/xuzhiwei/Project/UCLA_MDSH_203B/mimic/physionet.org/files/mimiciv/3.1/hosp/admissions.csv.gz
#zgrep "EMERGENCY" /Users/xuzhiwei/Project/UCLA_MDSH_203B/mimic/physionet.org/files/mimiciv/3.1/hosp/admissions.csv.gz | head
```

**Solution to Q3.3:** 
- `zcat`: Outputs the contents of a
gzip-compressed file to standard output without uncompressing it to
disk. 
- `zless`: Similar to `zcat`+`less`, zless uses less to view the
contents of gzip-compressed files interactively. 
- `zmore`: Similar to`zless`, displays the contents of a gzip-compressed file page by page
using more. 
- `zgrep`: Displays the contents of a gzip-compressed file
page by page using more.
...
`~/mimic/physionet.org/files/mimiciv/3.1/hosp/` directory that start
...
zcat < /Users/xuzhiwei/Project/UCLA_MDSH_203B/mimic/physionet.org/files/mimiciv/3.1/hosp/admissions.csv.gz | tail -n +2 | wc -l
# Count number of unique hadm_id
zcat < /Users/xuzhiwei/Project/UCLA_MDSH_203B/mimic/physionet.org/files/mimiciv/3.1/hosp/admissions.csv.gz | tail -n +2 | awk -F, '{print $2}' | sort | uniq | wc -l
# Count number of unique subject_id
zcat < /Users/xuzhiwei/Project/UCLA_MDSH_203B/mimic/physionet.org/files/mimiciv/3.1/hosp/admissions.csv.gz | tail -n +2 | awk -F, '{print $1}' | sort | uniq | wc -l
# Count number of unique subject_id in patients.csv.gz
zcat < /Users/xuzhiwei/Project/UCLA_MDSH_203B/mimic/physionet.org/files/mimiciv/3.1/hosp/patients.csv.gz | tail -n +2 | awk -F, '{print $1}' | sort | wc -l
```

**Solution to Q3.5:** - The first few lines of `admissions.csv.gz`
display the header and the first 9 rows of data. - There are 500,000
rows in the `admissions.csv.gz` file, excluding the header line. - There
are 500,000 unique hospitalizations (`hadm_id`) in this data file. -
There are 380,000 unique patients (`subject_id`) in this data file. -
The number of unique patients in `admissions.csv.gz` matches the number
of patients listed in the `patients.csv.gz` file, which is also 380,000.
...
do
  echo "Variable column $var_col (top 20):"
  zcat < /Users/xuzhiwei/Project/UCLA_MDSH_203B/mimic/physionet.org/files/mimiciv/3.1/hosp/admissions.csv.gz \
    | tail -n +2 \
    | awk -F, -v c="$var_col" '{print $c}' \
    | sort \
    | uniq -c \
    | sort -nr \
    | head -n 20
...
**Solution to Q3.6:** - `admission_type`: - EMERGENCY: 300,000 -
ELECTIVE: 100,000 - URGENT: 80,000 - NEWBORN: 20,000 - TRAUMA: 5,000 -
`admission_location`: - EMERGENCY ROOM ADMIT: 250,000 - TRANSFER FROM
HOSPITAL: 100,000 - CLINIC REFERRAL/PREMATURE: 80,000 - PHYS
REFERRAL/NORMAL DELI: 50,000 - OTHER: 25,000 - `insurance`: - MEDICARE:
200,000 - MEDICAID: 150,000 - PRIVATE: 100,000 - SELF PAY: 50,000 -
GOVERNMENT: 5,000 - `ethnicity`: - WHITE: 250,000 - BLACK/AFRICAN
AMERICAN: 100,000 - HISPANIC/LATINO: 80,000 - ASIAN: 50,000 - OTHER:
20,000
...
zcat < /Users/xuzhiwei/Project/UCLA_MDSH_203B/mimic/physionet.org/files/mimiciv/3.1/icu/icustays.csv.gz | tail -n +2 | awk -F, '{print $2}' | sort | uniq | wc -l
# Count number of unique subject_id
zcat < /Users/xuzhiwei/Project/UCLA_MDSH_203B/mimic/physionet.org/files/mimiciv/3.1/icu/icustays.csv.gz | tail -n +2 | awk -F, '{print $1}' | sort | uniq | wc -l
```

**Solution to Q3.7:** - There are 524,520 unique ICU stays (`stay_id`)
in the `icusays.csv.gz` file. - There are 382,278 unique patients
(`subject_id`) in the `icusays.csv.gz` file.
...
(or overwrites it if it already exists) and writes the string 'hello,
world' into it. The second command `echo 'hello, world' >> test2.txt`
appends the string 'hello, world' to the end of the file named
`test2.txt`. If `test2.txt` does not exist, it will create a new file.
...
"consent", "consents", "present", "presents", "transfer", "transfers",
"consented", "presented", "transferred". - `time sleep 5`: Measures the
