...
- `zless`: This command allows you to view the contents of compressed files one screen at a time, similar to the `less` command for uncompressed files. It is useful for reading large compressed files.
- `zmore`: Similar to `zless`, this command allows you to view the contents of compressed files one screen at a time, but it uses the `more` command style of pagination.
- `zgrep`: This command is used to search for specific patterns within compressed files. It works like the `grep` command but operates on .gz files directly.
...
```{bash}
#| eval: false
for datafile in ~/mimiciv/3.1/hosp/{a,l,pa}*.gz
do
  ls -l $datafile
done
```
